...
import html
import requests
import SophiaBot.modules.sql.kuki_sql as sql
...
from JisooX.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Koro Sensei Chatbot enable by {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
__help__ = """
Kaguya has a special feature that lets the bot talk and provide a more interactive group chat experience.
